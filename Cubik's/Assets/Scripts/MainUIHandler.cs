...
public class MainUIHandler : MonoBehaviour {
	...
	public GameObject warning, fillColorsSettings;
	...
	bool holdUnInteractable = false, startRotating = false, solvedUsingGuide = false;
	void Update() {
		if(!rl.isRotating && rl.queue.Count == 0) {
			...
			if(solvedUsingGuide) {
				...
				currentMoves.GetComponent<Text>().text = solverHeaderText.text = "Solved!";
				if(cfopMethod) {
